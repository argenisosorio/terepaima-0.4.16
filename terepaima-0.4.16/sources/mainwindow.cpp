...
{
    QString about1,about2,about3,about4,about5,about6;
    about1 = trUtf8("Terepaima es una lector de documento PDF que usa QT. ");
    about2 = trUtf8("Pueden visualizar el código del ");
    about3 = trUtf8("Para mayor información ver ");
    about4 = trUtf8("Esta aplicación esta basada en qpdfview ");
    about5 = trUtf8("Centro Nacional de Desarrollo e Investigación en Tecnologías Libres 2016");
    about6 = trUtf8("Permite verificar y mostrar detalles de la firma electrónica del documento");
    QMessageBox::about(this, tr("Acerca de terepaima"), (tr("<p><b>Terepaima %1</b></p><p>").arg(APPLICATION_VERSION) + about1 + tr("</p><p>") + about6 + tr("</p><p>") + about4 + tr(" <a href=\"https://launchpad.net/qpdfview\">launchpad.net/qpdfview</a></p>"
                                                      "<p>Esta versión incluye:"
                                                      "<ul>")
#ifdef WITH_PDF
...
#endif // WITH_CUPS
                                                      + tr("</ul>"
                                                           "<p> Equipo de trabajo: <br> <b> ") + about5 + tr(" </b> <a href=\"http://www.cenditel.gob.ve/\">CENDITEL</a></p>")
                                                      + tr("<ul><li>Pedro Buitrago pbuitrago@cenditel.gob.ve</li>")
                                                      + tr("<li>Victor Bravo vbravo@cenditel.gob.ve</li>")
                                                      + tr("<li>Antonio Araujo araujo@cenditel.gob.ve</li>")
                                                      + tr("<li>Argenis Osorio aosorio@cenditel.gob.ve</li>")
                                                      + tr("<li>Jose Sulbaran jsulbaran@cenditel.gob.ve</li>")
                                                      + tr("<li>Cipriano Alvarado calvarado@cenditel.gob.ve</li></u></p>")
                                                      + tr("<p>") + about3 + tr("<a href=\"https://tibisay.cenditel.gob.ve/visorpdf\">terepaima</a></p>")
                                                      + tr("<p>") + about2 + tr("<a href=\https://tibisay.cenditel.gob.ve/visorpdf/browser/visorpdf\>Proyecto</a></p>")));
}
...
//    setWindowTitle(tabText + QLatin1String("qpdfview") + instanceName);
    setWindowTitle(tabText + QLatin1String("Terepaima") + instanceName);
}
...
void MainWindow::createActions()
{
    ...
    m_verify_signature = createAction(trUtf8("&Verificar firma electrónica..."), QLatin1String("Verify signature"), QLatin1String("verify-signature"), QKeySequence(), SLOT(on_verify_signature()));
...
void MainWindow::createDocks()
{
    ...
    // verify signature
    m_detailsSignatureDock = createDock(trUtf8("&Verificar firma electrónica"), QLatin1String("verifySignature-Dock"), QKeySequence(Qt::Key_F12));
    ...
    m_detailsSignatureView->verticalHeader()->hide();
    ...
    if (!doc->isOk()) {
     ...
     if( sigCount >= 1 ) { //El documento tiene firma electronica
         ...
         for (unsigned int i = 0; i < sigCount; i++) {
              ...
              model->setItem(countRow, 1, item);
              countRow ++;
              model->takeVerticalHeaderItem(countRow);
          }
         return model;
      }
...
QStandardItemModel *MainWindow::verify_signature() {
    ...
    if (!doc->isOk()) {
     ...
     QStandardItemModel* model = new QStandardItemModel(numRows, numColumns);
     model->setHorizontalHeaderLabels(horzHeaders);
     qDebug()<<horzHeaders[0];
     qDebug()<<horzHeaders[1];
    // model->setHorizontalHeaderLabels(horzheaders);
      QStringList itens;
      //QStandardItem* item;
      QString rowtype, rowvalue;
...
//        }
    ...
    int cont = 1;
    if(sigCount == 0) {
        my_msg_Box.setWindowTitle("Terepaima");
